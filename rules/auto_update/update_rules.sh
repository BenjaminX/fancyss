...
if [ -f "gfwlist_download.conf" ]; then
	cat gfwlist_download_tmp.conf gfwlist_koolshare.conf | grep -Ev "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | gsed "s/^/server=&\/./g" | gsed "s/$/\/127.0.0.1#7913/g" >gfwlist_merge.conf
	cat gfwlist_download_tmp.conf gfwlist_koolshare.conf | gsed "s/^/ipset=&\/./g" | gsed "s/$/\/gfwlist/g" >>gfwlist_merge.conf
fi
...
# delete site below
gsed -i '/m-team/d' "gfwlist1.conf"
gsed -i '/windowsupdate/d' "gfwlist1.conf"
gsed -i '/v2ex/d' "gfwlist1.conf"

md5sum1=$(md5sum gfwlist1.conf | gsed 's/ /\n/g' | gsed -n 1p)
md5sum2=$(md5sum ../gfwlist.conf | gsed 's/ /\n/g' | gsed -n 1p)
...
# curl https://raw.githubusercontent.com/firehol/blocklist-ipsets/master/ipip_country/ipip_country_cn.netset | gsed '/^#/d' >chnroute1.txt
curl https://raw.githubusercontent.com/PaPerseller/chn-iplist/master/chnroute.txt | gsed '/^#/d' >chnroute1.txt

md5sum3=$(md5sum chnroute1.txt | gsed 's/ /\n/g' | gsed -n 1p)
md5sum4=$(md5sum ../chnroute.txt | gsed 's/ /\n/g' | gsed -n 1p)
...
if [ "$md5sum3"x = "$md5sum4"x ]; then
	...
	gsed -i "2c $(date +%Y-%m-%d) # $md5sum3 chnroute" ../version1
fi
echo =================
# ======================================
...
if [ "$md5sum5"x = "$md5sum6"x ]; then
	...
	gsed -i "4c $(date +%Y-%m-%d) # $md5sum5 cdn" ../version1
fi
echo =================
# ======================================
# use apnic data
...
md5sum9=$(md5sum Routing.txt | gsed 's/ /\n/g' | gsed -n 1p)
md5sum10=$(md5sum ../Routing.txt | gsed 's/ /\n/g' | gsed -n 1p)
echo =================
if [ "$md5sum9"x = "$md5sum10"x ]; then
	...
	echo update Routing!
	cp Routing.txt ..
	gsed -i "5c $(date +%Y-%m-%d) # $md5sum9 Routing" ../version1
fi
echo =================
# ======================================
...
md5sum7=$(md5sum WhiteList.txt | gsed 's/ /\n/g' | gsed -n 1p)
md5sum8=$(md5sum ../WhiteList.txt | gsed 's/ /\n/g' | gsed -n 1p)
echo =================
if [ "$md5sum7"x = "$md5sum8"x ]; then
	...
	gsed -i "6c $(date +%Y-%m-%d) # $md5sum7 WhiteList" ../version1
fi
echo =================
...
md5sum11=$(md5sum WhiteList_new.txt | gsed 's/ /\n/g' | gsed -n 1p)
md5sum12=$(md5sum ../WhiteList_new.txt | gsed 's/ /\n/g' | gsed -n 1p)
echo =================
if [ "$md5sum11"x = "$md5sum12"x ]; then
	...
	gsed -i "7c $(date +%Y-%m-%d) # $md5sum11 WhiteList_new" ../version1
fi
echo =================

# ======================================
# get cdn list for shadowsocks chn and game mode
cat apple.china.conf | gsed '/^#/d' | gsed "s/server=\/\.//g" | gsed "s/server=\///g" | gsed -r "s/\/\S{1,30}//g" | gsed -r "s/\/\S{1,30}//g" | sort -u > apple_download.txt
cat google.china.conf | gsed '/^#/d' | gsed "s/server=\/\.//g" | gsed "s/server=\///g" | gsed -r "s/\/\S{1,30}//g" | gsed -r "s/\/\S{1,30}//g" | sort -u > google_download.txt

md5sum13=$(md5sum apple_download.txt | gsed 's/ /\n/g' | gsed -n 1p)
md5sum14=$(md5sum ../apple_china.txt | gsed 's/ /\n/g' | gsed -n 1p)

md5sum15=$(md5sum google_download.txt | gsed 's/ /\n/g' | gsed -n 1p)
md5sum16=$(md5sum ../google_china.txt | gsed 's/ /\n/g' | gsed -n 1p)
...
if [ "$md5sum13"x = "$md5sum14"x ]; then
	...
	gsed -i "8c $(date +%Y-%m-%d) # $md5sum13 apple_china" ../version1
fi
if [ "$md5sum15"x = "$md5sum16"x ]; then
	...
	gsed -i "9c $(date +%Y-%m-%d) # $md5sum15 google_china" ../version1
fi
echo =================
# ======================================
